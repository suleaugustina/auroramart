...
FROM raw_events WHERE event_type = 'product.viewed'
UNION ALL
SELECT
    '2. Added to Cart' as step,
    COUNT(DISTINCT session_id) as unique_sessions
FROM raw_events WHERE event_type = 'cart.item_added'
UNION ALL
SELECT
    '3. Checkout Started' as step,
    COUNT(DISTINCT session_id) as unique_sessions
FROM raw_events WHERE event_type = 'checkout.started'
UNION ALL
SELECT
    '4. Order Placed' as step,
    COUNT(DISTINCT session_id) as unique_sessions
FROM raw_events WHERE event_type = 'order.placed'
UNION ALL
SELECT
    '5. Order Paid' as step,
    COUNT(DISTINCT session_id) as unique_sessions
FROM raw_events WHERE event_type = 'order.paid';
...
FROM (
    SELECT payment_method, event_type FROM raw_events WHERE event_type IN ('order.paid', 'payment.failed') AND payment_method IS NOT NULL
) sub
